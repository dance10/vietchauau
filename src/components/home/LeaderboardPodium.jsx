...
function PodiumCard({ item, isTop }) {
  ...
  return (
    <div className={`bg-surface rounded-xl shadow-sm border border-border-light ${c.border} ${c.glow} p-4 md:p-8 flex flex-col items-center ${isTop ? "w-36 md:w-56 z-10 -translate-y-4 md:-translate-y-6" : "w-28 md:w-40"} hover:-translate-y-2 transition-transform duration-300`} style={{ borderTopWidth: "4px" }}>
      {isTop && <div className="absolute -top-8 text-gold"><Crown className="w-8 h-8 md:w-10 md:h-10 drop-shadow-md" /></div>}
      <div className={`rounded-full flex items-center justify-center font-black border-2 ${isTop ? "w-16 h-16 md:w-20 md:h-20 bg-secondary-container text-on-secondary-container border-gold shadow-md mt-4 text-lg md:text-xl" : "w-12 h-12 md:w-16 md:h-16 bg-surface-container-highest text-text-muted border-outline-variant text-sm md:text-base"}`}>
        {item.initials}
      </div>
...
export default function LeaderboardPodium({ podium }) {
  ...
  return (
    <div className="flex items-end justify-center gap-3 md:gap-6 pt-12 md:pt-14 mb-8 md:mb-12">
