...
## Getting Supermemory API Key

1. **Sign up**: Visit [console.supermemory.ai](https://console.supermemory.ai) and create a free account
2. **Generate key**: Navigate to **API Keys** → **Create API Key**
3. **Copy**: The key will look like `sm_xxxxxxxxxxxxxxxx`
4. **Set env**: `$env:SUPERMEMORY_API_KEY = "sm_your_key_here"` (Windows) or `export SUPERMEMORY_API_KEY="sm_your_key_here"` (macOS/Linux)

## MCP Server Configuration

Supermemory MCP server is pre-configured in `.kilo/kilo.json` but **disabled by default**.
...
1. Edit `.kilo/kilo.json`:
   ...
   ```
2. Replace `sm_your_api_key_here` with your actual API key from step 3 above
