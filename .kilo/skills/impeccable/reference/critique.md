...
2. **Compute the slug**:
   ```bash
   node .kilo/skills/impeccable/scripts/critique-storage.mjs slug "<resolved-path-or-url>"
   ```
...
CLI scan:
```bash
node .kilo/skills/impeccable/scripts/detect.mjs --json [target]
```
...
4. If mutation is available, start `node .kilo/skills/impeccable/scripts/live-server.mjs --background`, present the browser if supported, label `[Human]`, scroll top, inject `http://localhost:PORT/detect.js`, wait 2-3 seconds, read `impeccable` console messages, then stop the live server.
...
Auto-select 2-3 personas most relevant to this interface type (use the selection table in the reference). If `.kilo/settings.json` contains a `## Design Context` section from `impeccable init`, also generate 1-2 project-specific personas from the audience/brand info.
...
2. **Pass the structured metadata** through `IMPECCABLE_CRITIQUE_META` (JSON), then run the write command:
   ...
   IMPECCABLE_CRITIQUE_META='{"target":"<user phrasing>","total_score":<n>,"p0_count":<n>,"p1_count":<n>}' \
     node .kilo/skills/impeccable/scripts/critique-storage.mjs write <slug> <body-file>
   ```
...
4. **Read the trend** for context:
   ```bash
   node .kilo/skills/impeccable/scripts/critique-storage.mjs trend <slug> 5
   ```
...
If `.kilo/settings.json` contains a `## Design Context` section (generated by `impeccable init`), derive 1–2 additional personas from the audience and brand information:
